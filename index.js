...
app.use(cors({
    origin: ['http://127.0.0.1:5500', 'http://127.0.0.1:5502']
}));
...
app.post('/profesores', (req, res) => {
    ...
    function insertProfesor() {
        ...
        db.query(query, { nombre, apellido, email, contraseña, id_grado_asignado }, (err, result) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
...
app.get('/profesores/:id/alumnos', (req, res) => {
    ...
    const getGradoQuery = 'SELECT id_grado_asignado FROM Profesores WHERE id_profesor = ?';
    ...
    db.query(getGradoQuery, [id], (err, profesorResults) => {
        ...
        if (!id_grado) {
            return res.status(400).json({ error: 'Este profesor no tiene un grado asignado' });
        }
...
app.put('/profesores/:id/grado', (req, res) => {
    ...
    db.query(checkGradoQuery, [id_grado_asignado], (err, gradoResults) => {
        ...
        const updateQuery = 'UPDATE Profesores SET id_grado_asignado = ? WHERE id_profesor = ?';
...
app.post('/grados', (req, res) => {
    ...
    if (!nombre_grado || !nivel) {
        return res.status(400).json({ error: 'Nombre y nivel del grado son requeridos' });
    }
...
app.post('/alumnos', (req, res) => {
    ...
    if (!nombre || !apellido || !grado) {
        return res.status(400).json({ error: 'Nombre, apellido y grado son requeridos' });
    }
    ...
    db.query(findGradoQuery, [grado], (err, gradoResults) => {
        ...
        // Insertar el alumno
        const insertQuery = 'INSERT INTO Alumnos (nombre, apellido, id_grado) VALUES (?, ?, ?)';
...
app.get('/alumnos/:id/asistencia', (req, res) => {
    const { id } = req.params;
    const query = 'SELECT * FROM Asistencia WHERE id_alumno = ? ORDER BY fecha DESC';
    db.query(query, [id], (err, results) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
    ...
    try {
        ...
        for (const asistencia of asistencias) {
            ...
            if (!estadosPermitidos.includes(asistencia.estado.toLowerCase())) {
                return res.status(400).json({ 
                    error: `Estado '${asistencia.estado}' no permitido. Use: ${estadosPermitidos.join(', ')}`
                });
            }
        }
...
app.put('/asistencia/update-multiple', (req, res) => {
        ...
        if (asistencia.id_asistencia) {
            updates.push(
                db.query(
                    ...
                    [asistencia.estado, asistencia.comentario || '', asistencia.id_asistencia]
                )
            );
            ...
            updates.push(
                db.query(
                    ...
                    [asistencia.id_alumno, asistencia.fecha, asistencia.estado, asistencia.comentario || '']
                )
            );
        }
    });
    ...
    Promise.all(updates)
        .then(() => {
            res.json({ success: true, message: 'Asistencias actualizadas correctamente' });
        })
        .catch(error => {
